...
main <- function() {
  ...
  if (! all(c("expr.mtx", "var.csv", "cells.txt") %in% list.files(file.path(args$dir, "expr_info")))) {
    stop(paste0("Error: The directory ", args$dir, " does not contain the right files (expr.mtx, var.csv, cells.txt)"))
  }
  ...
  if (nrow(expr) != nrow(var_df)) {
    ...
    if (!is.null(args$sponge_network) & !is.null(args$sponge_analysis) & (args$aucell | args$gsva | args$ssgsea)) {
      ...
      if (!(right_order | wrong_order)) {
        ...
        if (length(names(sponge_modules)) == 0) {
          ...
          log_message("Calculating scores with the SPONGE network ...", logfile)
    ...
    if (! is.null(args$genie_network)) {
      ...
      if (length(names(regulons)) == 0) {
        ...
        log_message("Calculating scores with the GENIE network ...", logfile)
