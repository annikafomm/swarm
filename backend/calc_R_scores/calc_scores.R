...
main <- function() {
  ...
  if (! all(c("expr.mtx", "var.csv") %in% list.files(file.path(args$dir, "expr_info")))) {
  ...
  ### check?
  if (! (args$aucell | args$gsva | args$ssgsea | args$viper)) {
  ...
  if (! is.null(args$sponge_network) & !is.null(args$sponge_analysis) & (args$aucell | args$gsva | args$ssgsea)) {
    ...
    if (! args$ensembl_col %in% colnames(var_df)) {
      stop("You provided the wrong column for the ensembl ids.")
    }
  ...
  if (! is.null(args$genie_network)) {
    ...
    if (modules_exist & (args$gsva | args$ssgsea | args$aucell)) {
