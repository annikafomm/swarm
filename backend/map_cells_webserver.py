...
def select_genes(ad_sc, ad_sp, selection_mode):
    ...
    if selection_mode == "ctg":
        genes = gene_selection.ctg(ad_sc, "cell_type")
        print("ctg")
        ...
        genes = gene_selection.spapros(ad_sc, "cell_type")
        print("spapros")
...
def main(args):
    ...
    # Mapping
    ad_map = tg.map_cells_to_space(ad_sc, ad_sp, mode = "clusters", cluster_label='cell_type')

    # Project gene expression
    ad_ge = tg.project_genes(ad_map, ad_sc, cluster_label='cell_type')
    ad_ge.obsm["spatial"] = ad_sp.obsm["spatial"]

    # Project cell annotations
    tg.project_cell_annotations(ad_map, ad_ge, annotation="cell_type")
