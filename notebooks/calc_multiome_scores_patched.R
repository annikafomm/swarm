...
# "Rscript" "../backend/calc_multiome_scores/calc_multiome_scores_patched.R"
...
plot_footprint_for_motif <- function(
  motif_name,
  M = NULL,
  M_path = NULL,              # e.g. adata_map.X.csv
  spot_obj = NULL,
  spot_obj_path = NULL,
  ...
  # --- overwrite behavior now applies ONLY to the plot file ---
  if (!is.null(plot_out_path) && !overwrite && file.exists(plot_out_path)) {
    stop("Refusing to overwrite existing plot file: ", plot_out_path)
  }
  ...
  # footprint key in Signac
  key <- sprintf("footprint_%s", motif_name)
  ...
  if (is.null(plot_out_path)) {
    # default next to spot_obj_path if available, else CWD
    out_dir <- if (!is.null(spot_obj_path)) dirname(spot_obj_path) else "."
    plot_out_path <- file.path(out_dir, paste0("footprint_", motif_name, ".pdf"))
  }

  ggplot2::ggsave(filename = plot_out_path, plot = p, width = 6, height = 4)

  # return both updated objects + plot
  return(list(
    plot = p,
    object_dissociated = object_dissociated,
    spot_obj = spot_obj,
...
global_motif_analysis <- function(object, args, logfile) {
  ...
  if (args$footprinting) {
    ...
    if (!is.null(args$footprinting_motifs) && nzchar(args$footprinting_motifs)) {
      ...
      top_tbl <- top_motifs_per_comparison(spot_obj, n = args$footprinting_top_n)
      motifs <- unique(top_tbl$motif)
    } else {
